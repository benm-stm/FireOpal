...
class TestSuite {
    ...
    public function run() {
        if ($this->_user) {
            exec('rspec '.$this->_testSuiteFile.' -r rspec_junit_formatter --format RspecJunitFormatter -o output.xml 2>&1', $output);
    ...
    public function bindRspecSetUp($rspecFileObj) {
        if ($rspecFileObj->isWritable()) {
            ...
            $content .= "        @params  = JSON.parse('".$set."')\n";
            $content .= "        @valid.setup(@params)\n";
            $content .= "        @valid.login()\n";
    ...
    public function bindConfigurationElements() {
        ...
        if ($this->_testSuiteFile->isWritable()) {
            ...
            $content .= "        @params  = setup\n";
            $content .= "        @runner = Selenium::WebDriver.for :remote,";
            $content .= " :url => \"http://#{@params['client']['value']}:4444/wd/hub\",";
            $content .= " :desired_capabilities => @params['browser']['value'].to_sym\n";
            $content .= "        @runner.get @params['host']['value']\n";
            $content .= "        @runner.manage.timeouts.implicit_wait = 30\n";
            $content .= "    end\n\n";
            $content .= "    def teardown\n        @runner.quit\n    end\n\n";
            $content .= "    def login\n";
            $content .= "        @runner.navigate.to @params['host']['value'] + '/my/'\n";
            $content .= "        @runner.find_element(:name, \"form_loginname\").send_keys @params['user']['value']\n";
            $content .= "        @runner.find_element(:name, \"form_pw\").send_keys @params['password']['value']\n";
